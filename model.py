...
def vgg16(level, batch_norm,num_class = 10):
    ...
    if level == 3:
        ...
        top_net += vgg16_make_layers([1024, 1024, "M"], in_channels=512)
        top_net += [nn.AdaptiveAvgPool2d((1, 1))]
        top_net += [nn.Flatten(),nn.Linear(1024 * 1 * 1, num_class)]
        return nn.Sequential(*client_net),nn.Sequential(*server_net), nn.Sequential(*top_net)
...
def cifar_mobilenet(bottom_level, top_level):
    client = []
    server = []
    top = []
    if bottom_level == 1:
        if top_level == 1:
            ...
            top += nn.Sequential(nn.AvgPool2d(1), nn.Flatten(), nn.Linear(1024, 10))
            return nn.Sequential(*client), nn.Sequential(*server), nn.Sequential(*top)
            ...
            top += conv_dw(512, 1024, 2)
            top += conv_dw(1024, 1024, 1)
            top += nn.Sequential(nn.AvgPool2d(1), nn.Flatten(), nn.Linear(1024, 10))
            return nn.Sequential(*client), nn.Sequential(*server), nn.Sequential(*top)
